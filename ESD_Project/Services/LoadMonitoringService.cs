...
namespace ESD_Project.Services
{
    public class LoadMonitoringService
    {
        ...
        public LoadMonitoringService(AppDbContext db) => _db = db;
        ...
        //public async Task<List<TimeSeriesPoint>> GetLoadTimeSeriesAsync(TimeSpan window)
        //{
        //    var cutoff = DateTime.UtcNow - window;

        //    // 1) Fetch the raw readings from the database
        //    var raw = await _db.LoadWeights
        //        .Where(x => x.Timestamp >= cutoff)
        //        .ToListAsync();      // now we're in-memory
